...
void Database_create(struct Connection* conn){
  ...
  for(i = 0; i < MAX_ROWS; i++){
    ...
    //why on stack?
    //because temp memory, popped when function returns
    conn->db->rows[i] = addr;
  }
}
...
void Database_find(struct Connection* conn, const char* text){
  int i = 0;
  struct Address temp[MAX_ROWS];
  int j = 0;

  for(i = 0; i < MAX_ROWS; i++){
    struct Address* addr = &conn->db->rows[i];
    if(addr->set){
      if(strstr(addr->name, text) || strstr(addr->email, text)){
        temp[j] = *addr;
        j++;
      }
    }
  ...
  if(j==0){
    die("Pattern not found", conn);
  }
  for(i = 0; i < j; i++){
    Address_print(&temp[i]);
  }
}
...
void Database_list(struct Connection* conn){
  int i = 0;
  struct Database* db = conn->db; for(i = 0; i < MAX_ROWS; i++){
...
int main(int argc, char* argv[]){
  ...
  if(id >= MAX_ROWS) die("There aren't that many recoreds.", NULL);
